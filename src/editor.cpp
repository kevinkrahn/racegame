...
void Editor::onUpdate(f32 deltaTime)
{
    ...
    cameraTarget += ((forward * moveDir.y) + (sideways * moveDir.x)) * deltaTime * 30.f;
    ...
    {
        cameraAngle += ((lastMousePosition.x) - game.input.getMousePosition().x) / game.windowWidth * 2.f;
        lastMousePosition = game.input.getMousePosition();
    }

    cameraDistance = clamp(cameraDistance - game.input.getMouseScroll() * 5.f, 10.f, 120.f);
